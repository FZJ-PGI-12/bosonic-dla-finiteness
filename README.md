...
Each entry requires `kind` (`"+"` or `"-"`), either `alpha`/`beta` or `iotas` (not both), and a `label`. The `description` field is optional. `label` and `description` are documentation only — they do not affect the classification.
