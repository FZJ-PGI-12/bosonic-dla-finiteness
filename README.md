...
$$\frac{\mathrm{d}}{\mathrm{d}t}U(t)=-\mathrm{i}H(t)U(t)\qquad\text{with}\qquad U(0)=\unity,$$
...
A finite set of generators $\mathcal{G}=\{g_{\sigma_k}^{\gamma_k}\}_{k\in \mathcal{K}}$ therefore admits a doisjoiint decomposition with components $\mathcal{G}^K:=\mathcal{G}\cap \hat{A}_n^K$, where $\mathcal{k}\in\{0,1,2,\mathrm{om},=,\perp\}$.
...
$$\chi_{\mathcal{F}}(\gamma) = \bigl(\chi_{X^{(1)}}(\gamma), \ldots, \chi_{X^{(m)}}(\gamma)\bigr)\qquad\text{with}\qquad \chi_{\vec{X}}(\gamma) = \sum_k x_k(\alpha_k - \beta_k)$$ 
...
3. Unresolved. In this case, the algorithm decomposes $\mathfrak{g}$ as a vector-space sum $\mathfrak{g}=\mathfrak{g}_{\mathrm{fin}}+\mathfrak{g}_{\mathrm{res}}$, where $\mathfrak{g}_{\mathrm{fin}}$ is a certified finite-dimensional Lie subalgebra contained in $\hat{A}_n^0\oplus\hat{A}_n^1\oplus\hat{A}_n^2\oplus\hat{A}_n^{\mathrm{om}}\oplus\hat{A}_n^{=}$, while $\mathfrak{g}_{\mathrm{res}}$ is a residual Lie subalgebra contained in ${\langle\hat{A}_n^2\oplus\hat{A}n^\perp\rangle}_{\mathrm{Lie}}$ and generated by terms that commute with every free Hamiltonian. The intersection $\mathfrak{g}_{\mathrm{fin}}\cap\mathfrak{g}_{\mathrm{res}}$ is abelian, but the algorithm does not determine whether $\mathfrak{g}_{\mathrm{res}}$—and hence $\mathfrak{g}$—is finite-dimensional.
